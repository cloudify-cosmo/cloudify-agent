...
function install_deps() {
	...
	if which apt-get; then
		...
		if [[ $(cat /etc/redhat-release) =~ "(Santiago)" ]];then
			sudo yum install -y http://mirror.centos.org/centos/6/os/x86_64/Packages/libffi-devel-3.0.5-3.2.el6.x86_64.rpm
		else
...
# 	sudo pip install "virtualenv>=14.0.0,<15.0.0" &&
	...
	sudo pip install https://github.com/cloudify-cosmo/cloudify-agent-packager/archive/CY-886.zip
}
